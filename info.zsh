...
echo "==========Fun=========="
echo 'npx say "The ships hung in the sky in much the same way that bricks don''t."'
echo 'npx benny-hill'
echo 'npx happy-birthday Andy'
printf "======================\n\n"
...
echo "===========Brew/Brew Cask==========="
echo "brew install FORMULA, brew list, brew search FORMULA, brew info FORMULA, brew outdated, brew upgrade FORMULA, brew cleanup FORMULA, brew uninstall FORMULA --force, brew --cellar"
...
echo "david"
echo "live-server"
echo "artillery quick --count 10 -n 20 https://artillery.io/"
printf "========================\n\n"
...
echo "=============Images============="
echo "identify -verbose some.jpg | grep Quality - for compression"
echo "identify -verbose some.jpg | grep Interlace - If JPEG then progressive, if None then baseline"
echo "identify -format '%[channels]' some.png - If ends with 'a' then has alpha channel"
echo "convert some.png -verbose info: - Trailing colon is important. Look at Alpha mean to see how transparent"
...
echo "cmd+shift+p command menu"
echo "When paused while debugging, hold down cmd and then click to new line to execute to that point"
echo "***Atom***"
...
echo "npx babel-node --debug --presets es2015 -- script.js --debug"
printf "========================\n\n"

echo "===========Git=========="
echo "You can reference commits by their SHA, by tags, branches, and the special HEAD pointer. Meaning <commit> in the following commands can take any of those values."
...
echo "git branch <branch> <commit> (optionally supply <commit> to branch off from previous commit)"
echo "git checkout <branch>"
echo "git branch --move <branch-from> <branch-to>"
echo "git branch --remotes"
echo "git branch -d|-D <branch> (short for --delete and --delete --force)"
echo "When on master running 'git merge <branch>' merges <branch> in to master"
echo "git merge <branch> for a fast forward merge, git merge <branch> -m for a regular merge"
echo "git config --list --local|--global|--system"
echo "Local config is at .git/config"
echo "Global config is at ~/.gitconfig"
echo ".git/objects stores all commits"
echo ".git/refs holds pointers to commits (basically the 'branches' and 'tags')"
echo ""
echo "There are two different types of branches, a local branch and a remote tracking branch."
echo "git remote --verbose"
echo "git remote add <name> <url>"
echo "git fetch <repo> <branch> is like git pull except it doesn't automatically merge the remote branch into your current local branch"
echo "git shortlog --numbered --summary"
echo "git log --author=<part-of-name> --oneline"
echo "git log --grep <pattern>"
echo "git remote rename <branch-from> <branch-to>"
echo "git rebase --interactive HEAD~3 enables us to squash the last 3 commits (have to choose 'squash' option though). HEAD~3 is just one type of <commit> form you can use. Create a backup branch before rebasing."
echo "git blame <filename>"
printf "======================\n\n"